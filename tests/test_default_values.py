...
class TestDefaultValues:
    ...
    def test_all_default_values_with_minimal_input(self, tmpdir, databricks_cli, cloud):
        ...
        for param, expected_value in expected_defaults.items():
            assert f"{param}={expected_value}" in test_file_contents, f"Missing or incorrect: {param}={expected_value}"
    ...
    def test_defaults_consistency_across_generation_types(self, tmpdir, databricks_cli):
        ...
        context_full = {**shared_params, "input_setup_cicd_and_project": "CICD_and_Project"}
        generate(full_dir, databricks_cli, context=context_full)
        full_contents = (full_dir / TEST_PROJECT_NAME / "_params_testing_only.txt").read_text("utf-8")
        
        # Test Project_Only defaults  
        project_dir = tmpdir.mkdir("project")
        context_project = {**shared_params, "input_setup_cicd_and_project": "Project_Only"}
        generate(project_dir, databricks_cli, context=context_project)
        project_contents = (project_dir / TEST_PROJECT_NAME / "_params_testing_only.txt").read_text("utf-8")
        ...
        cicd_contents = (cicd_dir / TEST_PROJECT_NAME / "_params_testing_only.txt").read_text("utf-8")
        ...
        for param in shared_default_params:
            ...
            if "input_setup_cicd_and_project=Project_Only" not in project_contents:
                assert param in project_contents  # Some params might be skipped for Project_Only
    ...
    def test_parameter_templating_in_defaults(self, tmpdir, databricks_cli):
        ...
        assert f"input_root_dir={TEST_PROJECT_NAME}" in test_file_contents  # Default: {{ .input_project_name }}
        assert "input_schema_name=schema_name" in test_file_contents  # Default schema name
        assert "input_inference_table_name=dummy.schema.table" in test_file_contents  # Default inference table
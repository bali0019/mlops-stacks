...
def test_azure_devops_pipeline_folder_structure(
    cicd_platform, setup_cicd_and_project, include_feature_store, 
    include_mlflow_recipes, include_models_in_unity_catalog, cloud, generated_project_dir
):
    ...
    project_dir = generated_project_dir / "my-mlops-project"  # This is what the fixture actually generates
...
def test_azure_devops_pipeline_files_exist(
    ...
    for pipeline_file in expected_files:
        ...
        assert pipeline_path.exists(), f"Azure DevOps pipeline {pipeline_file} should exist"
        
        # Verify YAML syntax is valid
        with open(pipeline_path, 'r') as f:
            pipeline_config = yaml.safe_load(f)
        assert pipeline_config is not None, f"Pipeline {pipeline_file} should have valid YAML"


@pytest.mark.parametrize("cicd_platform", ["azure_devops"])
@parametrize_by_cloud
def test_azure_devops_pipeline_structure(
...
def test_azure_devops_test_pipeline_structure(
    ...
    assert "jobs" in test_config or "stages" in test_config, "Test pipeline should define jobs or stages"
    ...
    if "trigger" in bundle_config:
        ...
        if isinstance(trigger_config, dict) and "branches" in trigger_config:
            ...
            if isinstance(branches, dict) and "include" in branches:
                ...
                assert context["input_default_branch"] in str(included_branches) or context["input_release_branch"] in str(included_branches)
...
def test_azure_devops_pipeline_has_deployment_stages(
    ...
    if "stages" in bundle_config:
        ...
        deployment_stages = [name for name in stage_names if "cd" in name.lower() or "deploy" in name.lower()]
        assert len(deployment_stages) > 0, f"Should have deployment stages. Found stages: {stage_names}"


@pytest.mark.parametrize("cicd_platform", ["azure_devops"])
def test_azure_devops_pipeline_has_variables(
    cicd_platform, tmpdir, databricks_cli
):
...
@pytest.mark.parametrize("cicd_platform", ["azure_devops"])
def test_azure_devops_deploy_pipeline_structure(
    cicd_platform, tmpdir, databricks_cli
):
...
@pytest.mark.parametrize("cicd_platform", ["azure_devops"])
def test_azure_devops_generates_readme(
    cicd_platform, tmpdir, databricks_cli
):
...
def test_azure_devops_pipeline_references_environments(
    ...
    assert "staging" in config_str.lower() or "prod" in config_str.lower(), \
...
def test_azure_devops_variable_replacement(
    ...
    assert "{{ .input_default_branch }}" not in tests_ci_content, "Template variables should be replaced"
    assert "{{ .input_project_name }}" not in tests_ci_content, "Template variables should be replaced"
    assert "{{template `project_name_alphanumeric_underscore` .}}" not in tests_ci_content, "Template functions should be replaced"
    
    # Assert correct values are present
    assert "main" in tests_ci_content, "Default branch should be replaced with 'main'"
    assert "my_mlops_project" in tests_ci_content, "Project name should be converted to alphanumeric underscore"
    assert "refs/heads/main" in tests_ci_content, "Branch references should include default branch"
    assert "my-mlops-project variable group" in tests_ci_content, "Variable group should reference project name"
    ...
    if cloud == "azure":
        ...
        assert "ARM_CLIENT_SECRET: $(STAGING_AZURE_SP_CLIENT_SECRET)" in tests_ci_content
    else:
    ...
    # Assert that template variables are replaced correctly
    assert "{{ .input_default_branch }}" not in bundle_ci_content, "Template variables should be replaced"
    assert "{{ .input_release_branch }}" not in bundle_ci_content, "Template variables should be replaced"
    assert "{{ .input_project_name }}" not in bundle_ci_content, "Template variables should be replaced"
    ...
    assert "refs/heads/main" in bundle_ci_content, "Default branch reference should be present"
    assert "refs/heads/release" in bundle_ci_content, "Release branch reference should be present"
    assert "my-mlops-project" in bundle_ci_content, "Project name should be present in display names"
    ...
    # Assert that template variables are replaced correctly
    assert "{{ .input_root_dir }}" not in deploy_ci_content, "Template variables should be replaced"
    assert "{{ .input_project_name }}" not in deploy_ci_content, "Template variables should be replaced"
    assert "{{template `cli_version` .}}" not in deploy_ci_content, "Template functions should be replaced"
    
    # Assert correct values are present
    assert "my-mlops-project variable group" in deploy_ci_content, "Variable group should reference root dir"
    assert "my-mlops-project" in deploy_ci_content, "Project name should be present as default parameter"
    ...
    if cloud == "azure":
        ...
        assert "ARM_CLIENT_SECRET: $(STAGING_AZURE_SP_CLIENT_SECRET)" in deploy_ci_content
    else:
...
def test_azure_devops_conditional_template_logic(
    ...
    assert "databricks bundle run write_feature_table_job -t test" in tests_ci_content, \
        "Feature store enabled should include write_feature_table_job step"
    assert "Run Feature Engineering Workflow for test deployment target" in tests_ci_content, \
    ...
    assert "{{ if (eq .input_include_feature_store `yes`) }}" not in tests_ci_content, \
        "Conditional template syntax should be processed"
    assert "{{ end }}" not in tests_ci_content, \
    ...
    assert "databricks bundle run write_feature_table_job -t test" not in tests_ci_content_no_fs, \
        "Feature store disabled should not include write_feature_table_job step"
    assert "Run Feature Engineering Workflow for test deployment target" not in tests_ci_content_no_fs, \
...
def test_azure_devops_generates_with_features_enabled(
    ...
    bundle_pipeline_path = pipelines_dir / f"{TEST_PROJECT_NAME}-bundle-cicd.yml"
    assert bundle_pipeline_path.exists(), "Pipeline should exist with feature store enabled"
    
    with open(bundle_pipeline_path, 'r') as f:
        bundle_config = yaml.safe_load(f)
    
    # Pipeline should still be valid YAML
    assert bundle_config is not None, "Pipeline should be valid YAML with features enabled"
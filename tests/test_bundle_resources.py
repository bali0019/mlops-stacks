...
class TestBundleResources:
    ...
    def test_databricks_yml_syntax_valid(
        ...
        databricks_yml_path = tmpdir / TEST_PROJECT_NAME / TEST_PROJECT_DIRECTORY / "databricks.yml"
    ...
    def test_resource_files_generated_correctly(
        ...
        resources_dir = tmpdir / TEST_PROJECT_NAME / TEST_PROJECT_DIRECTORY / "resources"
    ...
    def test_feature_store_resources_conditional_generation(self, tmpdir, databricks_cli):
        ...
        resources_dir_with_fs = tmpdir / "with_fs" / TEST_PROJECT_NAME / TEST_PROJECT_DIRECTORY / "resources"
        fs_resource_path = resources_dir_with_fs / "feature-engineering-workflow-resource.yml"
        assert fs_resource_path.exists(), "Feature store resource should exist when enabled"
        ...
        resources_dir_without_fs = tmpdir / "without_fs" / f"{TEST_PROJECT_NAME}_no_fs" / f"{TEST_PROJECT_NAME.replace('-', '_')}_no_fs" / "resources"
        fs_resource_path_no_fs = resources_dir_without_fs / "feature-engineering-workflow-resource.yml"
        assert not fs_resource_path_no_fs.exists(), "Feature store resource should not exist when disabled"

    def test_unity_catalog_resources_conditional_generation(self, tmpdir, databricks_cli):
        ...
        databricks_yml_path = tmpdir / "with_uc" / TEST_PROJECT_NAME / TEST_PROJECT_DIRECTORY / "databricks.yml"
        with open(databricks_yml_path, 'r') as f:
            bundle_config = yaml.safe_load(f)
        ...
        for target in ["dev", "staging", "prod"]:
            ...
            if "variables" in target_config:
                ...
                catalog_vars = [var for var in variables.keys() if "catalog" in var.lower()]
    ...
    def test_monitoring_resources_generated(self, tmpdir, databricks_cli):
        ...
        assert monitoring_resource_path.exists(), "Monitoring resource should be generated"
    ...
    def test_bundle_validation_with_databricks_cli(self, tmpdir, databricks_cli):
        ...
        assert result.returncode in [0, 1], f"Bundle validation failed unexpectedly: {result.stderr}"
        ...
        if result.returncode != 0:
            ...
            for indicator in syntax_error_indicators:
                assert indicator not in error_output, f"Bundle has syntax errors: {result.stderr}"

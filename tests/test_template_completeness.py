...
class TestTemplateCompleteness:
    ...
    def schema_params(self):
        ...
        schema_path = pathlib.Path(__file__).parent.parent / "databricks_template_schema.json"
    ...
    def test_all_schema_parameters_have_values(self, generated_project, schema_params):
        ...
        test_file_contents = (generated_project / "_params_testing_only.txt").read_text("utf-8")
        ...
        assert not missing_params, f"Missing parameters in generated project: {missing_params}"
    ...
    def test_template_variable_coverage(self, generated_project):
            ...
            if file_path.isfile():
                try:
                    ...
                    for match in all_template_matches:
                        ...
                        is_valid_cicd = any(re.search(pattern, match) for pattern in valid_cicd_patterns)
                        
                        # Report as unresolved if it looks like an MLOps template variable
                        if not is_valid_cicd and ('.input_' in match or match.startswith('{{ .')):
        ...
        assert not unresolved_vars, f"Found unresolved template variables: {unresolved_vars[:10]}"
    ...
    def test_parameter_enum_values_coverage(self, tmpdir, databricks_cli):
        ...
        for param, enum_values in enum_params.items():
            for enum_value in enum_values:
                ...
                elif param == "input_setup_cicd_and_project" and enum_value == "CICD_Only":
                    context["input_root_dir"] = context["input_project_name"]
                
                # Skip invalid combinations
                if (param == "input_include_mlflow_recipes" and enum_value == "yes" and
                    context.get("input_include_models_in_unity_catalog") == "yes"):
                    continue
                ...
                try:
                    ...
                    if enum_value != "CICD_Only":  # CICD_Only might not have _params_testing_only.txt
                        test_file = project_dir / context["input_project_name"] / "_params_testing_only.txt"
                        if test_file.exists():
    ...
    def test_all_cicd_platforms_generate_successfully(self, tmpdir, databricks_cli):
        ...
        cicd_platforms = ["github_actions", "github_actions_for_github_enterprise_servers", "azure_devops", "gitlab"]
    ...
    def test_feature_combinations_completeness(self, tmpdir, databricks_cli):
        ...
        for i, combo in enumerate(feature_combinations):
            ...
            fs_notebook = project_path / "feature_engineering" / "notebooks" / "GenerateAndWriteFeatures.py"
            if combo["feature_store"] == "yes":
                assert fs_notebook.exists(), f"Feature store notebook missing for combo {i}"
            else:
                assert not fs_notebook.exists(), f"Feature store notebook should not exist for combo {i}"
            
            # Check MLflow Recipes artifacts  
            recipes_notebook = project_path / "training" / "notebooks" / "TrainWithMLflowRecipes.py"
            if combo["mlflow_recipes"] == "yes":
                assert recipes_notebook.exists(), f"MLflow Recipes notebook missing for combo {i}"
            else:
                assert not recipes_notebook.exists(), f"MLflow Recipes notebook should not exist for combo {i}"
    ...
    def test_parameter_pattern_validation_coverage(self, tmpdir, databricks_cli):
        ...
        for param, test_values in test_cases.items():
            for value, should_succeed in test_values:
                ...
                try:
                    pattern_dir = tmpdir.mkdir(f"pattern_test_{param}_{abs(hash(value))}")
                    generate(pattern_dir, databricks_cli, context=context)
                    assert should_succeed, f"Expected {param}={value} to fail validation but it succeeded"
                except Exception:
                    assert not should_succeed, f"Expected {param}={value} to succeed validation but it failed"

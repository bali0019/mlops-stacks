...
class TestEdgeCases:
    ...
    def test_schema_name_validation(self, tmpdir, databricks_cli, schema_name, should_fail):
    ...
    def test_inference_table_name_validation(self, tmpdir, databricks_cli, inference_table_name, should_fail):
        ...
        if should_fail:
            with pytest.raises(Exception):  # CLI validation should reject invalid table names
                generate(tmpdir, databricks_cli, context=context)
        else:
            generate(tmpdir, databricks_cli, context=context)
            test_file_contents = (
            ...
            assert f"input_inference_table_name={inference_table_name}" in test_file_contents

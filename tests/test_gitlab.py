...
def test_generated_gitlab_folder(
    cicd_platform, cloud, include_models_in_unity_catalog, setup_cicd_and_project, generated_project_dir
):
    ...
    if setup_cicd_and_project != "CICD_Only":
        assert "bundle-cd-staging.yml" in pipeline_output, "Should contain staging CD pipeline file"
        assert "bundle-cd-prod.yml" in pipeline_output, "Should contain production CD pipeline file"
...
@parametrize_by_cloud
def test_gitlab_docker_configuration(
    cloud, tmpdir, databricks_cli
):
...
def test_gitlab_pipeline_stages_and_jobs(
    ...
    project_dir = tmpdir / TEST_PROJECT_NAME
    ci_pipeline_path = project_dir / ".gitlab" / "pipelines" / f"{TEST_PROJECT_NAME}-bundle-ci.yml"
    ...
    if "stages" in ci_config:
        ...
        assert "unit-test" in ci_config or "integration-test" in ci_config, "CI pipeline should have test job definitions"
        if "unit-test" in ci_config:
            assert "stage" in ci_config["unit-test"], "unit-test job should have a stage"
        if "integration-test" in ci_config:
            assert "stage" in ci_config["integration-test"], "integration-test job should have a stage"


def test_gitlab_environment_variables(
    tmpdir, databricks_cli
):
    ...
    assert "input_docker_image=databricksfieldeng/mlopsstacks:latest" in test_file_contents


@parametrize_by_cloud
def test_gitlab_trigger_pipeline_exists(
    cloud, tmpdir, databricks_cli
):
    ...
    project_dir = tmpdir / TEST_PROJECT_NAME
    trigger_pipeline_path = project_dir / ".gitlab" / "pipelines" / f"{TEST_PROJECT_NAME}-triggers-cicd.yml"
...
def test_gitlab_cd_pipelines_deployment_stages(
    tmpdir, databricks_cli  
):
    ...
    staging_cd_path = project_dir / ".gitlab" / "pipelines" / f"{TEST_PROJECT_NAME}-bundle-cd-staging.yml"
    ...
    assert "stages" in staging_config or "deploy-stage" in staging_config, "Staging CD should define stages or deployment jobs"
    
    # Test production CD pipeline
    prod_cd_path = project_dir / ".gitlab" / "pipelines" / f"{TEST_PROJECT_NAME}-bundle-cd-prod.yml"
    with open(prod_cd_path, 'r') as f:
        prod_config = yaml.safe_load(f)
    
    # Check for either stages or job definitions with stage properties
    assert "stages" in prod_config or "deploy-prod" in prod_config or "deploy-production" in prod_config, "Production CD should define stages or deployment jobs"


def test_gitlab_readme_documentation(
    tmpdir, databricks_cli
):
